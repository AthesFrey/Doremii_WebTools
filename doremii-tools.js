...
class DorePassword extends BaseTool {
  ...
  connectedCallback(){
    ...
    const ensureCapFirst = (s)=>{ return s.charAt(0).toUpperCase() + s.slice(1); };

    const gen = (L)=>{
      L = Math.max(4, Math.min(64, Number(L)||12));
      let out = CH.u[rnd(CH.u.length)] + CH.l[rnd(CH.l.length)] + CH.d[rnd(CH.d.length)] + CH.s[rnd(CH.s.length)];
      while(out.length < L) out += ALL[rnd(ALL.length)];
      out = shuffle(out.split('')).join('');
      out = ensureCapFirst(out);
      return out;
    };
